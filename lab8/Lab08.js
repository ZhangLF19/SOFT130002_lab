...
var alt=0;
var distance=0;
var alts=document.getElementsByClassName('buttons')[0].children;
var isOnPlay=true;
...
function nextPhoto() {
    ...
    alts[alt].setAttribute('class','');
    if (alt===4){
        alt=-1;
    }
   alt++;
    alts[alt].setAttribute('class','on');
...
function skip(i) {
    var x = -600 * i;
    var speace = 'left:' + x + 'px';
    distance = x;
    image.setAttribute('style', speace);
    alts[alt].setAttribute('class', '');
    alt = i;
    alts[alt].setAttribute('class', 'on')
    clearInterval(interval);
